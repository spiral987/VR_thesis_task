...
namespace Oculus.Interaction.DistanceReticles
{
    ...
    /// </summary>
    public class PersistentInteractionTubeVisual : PersistentInteractionLineVisual
    {
        [SerializeField]
        ...
        {
            //_tubeRenderer.Hide();
            _tubeRenderer.Show(); // 必要ならShow()を呼び出し続ける
        }

        protected override void HandleStateChanged(InteractorStateChangeArgs args)
        {
            base.HandleStateChanged(args);

            // Select状態でもRayを表示
            if (args.NewState == InteractorState.Select)
            {
                _shouldDrawLine = true; // Rayの描画を有効化
            }
        }
